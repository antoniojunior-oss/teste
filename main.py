...
with col_ia:
    ...
    if st.session_state.codigo_fonte:
        prompt_usuario = st.text_area("O que você quer mudar?", height=150)
        
        if st.button("✨ Aplicar/Otimizar Mudanças"):
            if model:
                with st.spinner("IA otimizando código..."):
                    contexto_html = st.session_state.codigo_fonte[:5000]
                    # Passamos o script atual para a IA refatorar
                    ...
                    2. Otimize o código: remova funções duplicadas, combine seletores CSS e garanta que o código seja limpo.
                    ...
                    try:
                        ...
                        # Sobrescreve com a versão otimizada em vez de somar
